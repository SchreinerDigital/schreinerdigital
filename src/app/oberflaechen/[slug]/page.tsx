...
export async function generateStaticParams() {
  const slugs = await getSlugs("oberflaechen");
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({
  params,
}: PageProps<"/oberflaechen/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const entry = await getEntry("oberflaechen", slug);
  if (!entry) return {};
  return {
    title: `${entry.meta.title} – Anwendung & Verarbeitung`,
    description: entry.meta.summary,
    alternates: { canonical: `/oberflaechen/${slug}` },
    openGraph: { type: "article", title: entry.meta.title, description: entry.meta.summary, images: [siteConfig.ogImage] },
  };
}

export default async function OberflaechePage({
  params,
}: PageProps<"/oberflaechen/[slug]">) {
  const { slug } = await params;
  const entry = await getEntry("oberflaechen", slug);
  if (!entry) notFound();

  const { default: Content, meta } = entry;
  const related = await getRelatedEntries("oberflaechen", meta, "kategorie");
  ...
  return (
    <Container className="py-12 sm:py-16">
      ...
      <RelatedPages basePath="/oberflaechen" heading="Weitere Oberflächen" items={related} />
    </Container>
  );
}
