...
export async function generateMetadata({
  params,
}: PageProps<"/verbindungstechnik/[slug]">): Promise<Metadata> {
...
export default async function VerbindungPage({
  params,
}: PageProps<"/verbindungstechnik/[slug]">) {
  const { slug } = await params;
  const entry = await getEntry("verbindungstechnik", slug);
  if (!entry) notFound();

  const { default: Content, meta } = entry;
  const related = await getRelatedEntries("verbindungstechnik", meta, "kategorie");
  ...
  return (
    <Container className="py-12 sm:py-16">
      ...
      <RelatedPages basePath="/verbindungstechnik" heading="Weitere Verbindungstechniken" items={related} />
    </Container>
  );
}
