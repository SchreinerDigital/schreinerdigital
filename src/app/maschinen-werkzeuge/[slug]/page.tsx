...
export async function generateStaticParams() {
  const slugs = await getSlugs("maschinen-werkzeuge");
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({
  params,
}: PageProps<"/maschinen-werkzeuge/[slug]">): Promise<Metadata> {
  const { slug } = await params;
  const entry = await getEntry("maschinen-werkzeuge", slug);
  if (!entry) return {};
  return {
    title: `${entry.meta.title} – Aufbau & Sicherheit`,
    description: entry.meta.summary,
    alternates: { canonical: `/maschinen-werkzeuge/${slug}` },
    openGraph: { type: "article", title: entry.meta.title, description: entry.meta.summary, images: [siteConfig.ogImage] },
  };
}

export default async function MaschineWerkzeugPage({
  params,
}: PageProps<"/maschinen-werkzeuge/[slug]">) {
  const { slug } = await params;
  const entry = await getEntry("maschinen-werkzeuge", slug);
  if (!entry) notFound();
