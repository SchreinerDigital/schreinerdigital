---
Serie: Auftragsabwicklung für Schreinereien (10/10)
Betreff: Stornorechnung – warum "einfach löschen" die schlechteste Idee ist
Preheader: Eine fehlerhafte, aber bereits gestellte Rechnung verschwindet nicht dadurch, dass du sie aus dem Ordner nimmst.
Verwandte Vorlage: Stornorechnung (/vorlagen)
---

# Die Stornorechnung: der saubere Weg aus einer falschen Rechnung

Zum Abschluss unserer Serie zur Auftragsabwicklung geht es um den Fall, den sich niemand wünscht: Eine bereits verschickte Rechnung ist grundlegend fehlerhaft – falscher Kunde, falscher Auftrag, versehentlich doppelt gestellt.

## Warum Löschen keine Option ist

Der naheliegende Impuls ist, die Rechnung einfach zu löschen oder zu vernichten, wenn sie noch nicht bezahlt wurde. Genau das ist steuerlich riskant: Sobald eine Rechnung ausgestellt und dem Kunden zugegangen ist, ist sie ein Beleg – ist sie bereits gebucht oder hat dein Kunde sie sogar schon zur Vorsteuererstattung genutzt, kann eine unrichtig ausgewiesene Umsatzsteuer nach § 14c UStG geschuldet bleiben, bis sie ordnungsgemäß korrigiert wurde. "Verschwinden lassen" storniert also nichts – es hinterlässt nur eine Lücke, die bei einer Prüfung auffällt.

Der korrekte Weg: die fehlerhafte Rechnung vollständig stornieren und, falls nötig, neu und korrekt ausstellen.

## So wird eine Stornorechnung richtig aufgebaut

- **Eindeutige Kennzeichnung** als "Stornorechnung" oder "Rechnungskorrektur" – nicht einfach als weitere normale Rechnung.
- **Eine eigene, neue fortlaufende Rechnungsnummer** – niemals die Nummer der stornierten Original-Rechnung wiederverwenden.
- **Klarer Bezug auf das Original**: Nummer und Datum der stornierten Rechnung müssen erkennbar sein.
- **Beträge als Abzug ausgewiesen**, typischerweise mit einem Minus-Vorzeichen (z. B. "./. 250,00 €") – inklusive der anteiligen, ebenfalls stornierten Umsatzsteuer.
- Bei Bedarf eine **neue, korrekte Rechnung** im Anschluss, mit wiederum eigener Rechnungsnummer.

## Stornorechnung oder Gutschrift – was jetzt?

Nach der letzten Ausgabe drängt sich die Frage auf: Storno oder Gutschrift? Die Faustregel: Ist die Rechnung als Ganzes falsch (falscher Empfänger, falscher Betrag, doppelt gestellt), brauchst du die vollständige Stornorechnung. Ist die Rechnung im Kern richtig und es geht nur um einen nachträglichen Teilnachlass, reicht die kaufmännische Gutschrift aus Ausgabe 9.

## Zum Abschluss dieser Serie

Vom Angebot bis zur Stornorechnung hat jedes Dokument in der Auftragsabwicklung seinen eigenen Zweck – und meistens auch seine eigene rechtliche Fallstrecke, wenn es fehlt oder falsch ausgefüllt ist. Die gute Nachricht: Einmal in eine solide Vorlage eingebaut, laufen die meisten dieser Punkte automatisch mit, ohne dass du bei jeder Rechnung neu daran denken musst.

## Auf den Punkt

Eine falsche Rechnung verschwindet nicht durchs Löschen – nur durch eine ordentliche Stornorechnung mit eigener Nummer, klarem Bezug zum Original und ausgewiesenem Minusbetrag.

---

Unsere **Stornorechnung-Vorlage** hat Nummerierung, Originalbezug und Minus-Ausweis bereits richtig vorbereitet. Kostenlos unter [schreiner.digital/vorlagen](/vorlagen) – wie alle zehn Vorlagen aus dieser Serie, im echten DIN-5008-Format und im schreiner.digital-Design.

Danke, dass du die ganze Serie mitgelesen hast. Falls eine Ausgabe besonders hilfreich war (oder du einen Fehler entdeckt hast): Antworte einfach auf diese Mail.

*Alle Angaben ohne Gewähr und keine Steuer- oder Rechtsberatung im Einzelfall.*
